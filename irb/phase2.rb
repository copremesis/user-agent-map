...
->(rails, dotnet) {
  #PropertyCompare.where("active = 1 and property_type like '%premium%' and status = '200' and phase2_score is NULL").each { |row|
  #PropertyCompare.where("active = 1 and property_type like '%premium%' and status = '200'").each { |row|
  #PropertyCompare.where("active = 1 and property_type like '%premium%' and status = '200' and phase2_score<=83").order(:phase2_score).each { |row|

 PropertyCompare.where(:phase2_score => 50).each {|row|
   begin
      @report_res = {}
...
}['http://ahl3rubys.apts.classifiedventures.com','http://www.apartmenthomeliving.com']
